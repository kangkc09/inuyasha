...
public class InuyashaPlayPage extends JFrame
{
	...
	private static final int[] PLAYER1_GUARD_POSITION_X =
	{ 25, 55, 88, 88, 55, 25 };
	private static final int[] PLAYER2_GUARD_POSITION_X =
	{ 7, 40, 70, 70, 40, 7 };
	private static final int[] PLAYER_GUARD_POSITION_Y =
	{ 22, 31, 22, 79, 97, 79 };
	private static final int PLAYER_ENUP_POSITION_X = 0;
	private static final int[] PLAYER_ENUP_POSITION_Y =
	{ 32, 53, 74 };
	...
	public void setUI()
	{
		...
		pnlPlayer2HPBar = new JPanel()
		{
			public void paintComponent(Graphics g)
			{
				super.paintComponent(g);
				g.setColor(Color.red);
				g.fillRect(BAR_WIDTH - play.CalculateBarLength(BAR_WIDTH, play.getPlayer2Character().getHP()), 0,
						play.CalculateBarLength(BAR_WIDTH, play.getPlayer2Character().getHP()), 30);
				g.setColor(Color.black);
				g.drawRect(0, 0, BAR_WIDTH, BAR_HEIGHT);
			}
		};
		...
		pnlPlayer2ENBar = new JPanel()
		{
			public void paintComponent(Graphics g)
			{
				super.paintComponent(g);
				g.setColor(Color.orange);
				g.fillRect(BAR_WIDTH - play.CalculateBarLength(BAR_WIDTH, play.getPlayer2Character().getEN()), 0,
						play.CalculateBarLength(BAR_WIDTH, play.getPlayer2Character().getEN()), 30);
				g.setColor(Color.black);
				g.drawRect(0, 0, BAR_WIDTH, BAR_HEIGHT - 1);
			}
		};
		...
		JPanel pnlPlayer1FirstAction = new JPanel()
		{
			public void paintComponent(Graphics g)
			{
				if (play.getIsOpenActionList()[FIRST_LIST_INDEX])
					g.drawImage(imgsrc.srcComboCards(play.getPlayer1Character().getID(),
							play.getPlayer1Character().getActionsID()[FIRST_LIST_INDEX]), 0, 0, null);
				else
					g.drawImage(imgsrc.srcBackCard(), 0, 0, null);
			}
		};
		pnlPlayer1FirstAction.setBounds(actionListPositionX(PLAYER1, FIRST_LIST_INDEX), actionListPositionY(),
		...
		JPanel pnlPlayer1SecondAction = new JPanel()
		{
			public void paintComponent(Graphics g)
			{
				if (play.getIsOpenActionList()[SECOND_LIST_INDEX])
					g.drawImage(imgsrc.srcComboCards(play.getPlayer1Character().getID(),
							play.getPlayer1Character().getActionsID()[SECOND_LIST_INDEX]), 0, 0, null);
				else
					g.drawImage(imgsrc.srcBackCard(), 0, 0, null);
			}
		};
		pnlPlayer1SecondAction.setBounds(actionListPositionX(PLAYER1, SECOND_LIST_INDEX), actionListPositionY(),
				ACTION_SIZE_X, ACTION_SIZE_Y);
		...
		JPanel pnlPlayer1ThirdAction = new JPanel()
		{
			public void paintComponent(Graphics g)
			{
				if (play.getIsOpenActionList()[THIRD_LIST_INDEX])
					g.drawImage(imgsrc.srcComboCards(play.getPlayer1Character().getID(),
							play.getPlayer1Character().getActionsID()[THIRD_LIST_INDEX]), 0, 0, null);
				else
					g.drawImage(imgsrc.srcBackCard(), 0, 0, null);
			}
		};
		pnlPlayer1ThirdAction.setBounds(actionListPositionX(PLAYER1, THIRD_LIST_INDEX), actionListPositionY(),
				ACTION_SIZE_X, ACTION_SIZE_Y);
		...
		JPanel pnlPlayer2FirstAction = new JPanel()
		{
			...
			{
				if (play.getIsOpenActionList()[FIRST_LIST_INDEX])
					g.drawImage(imgsrc.srcComboCards(play.getPlayer2Character().getID(),
							play.getPlayer2Character().getActionsID()[FIRST_LIST_INDEX]), 0, 0, null);
				else
					g.drawImage(imgsrc.srcBackCard(), 0, 0, null);
			}
		};
		pnlPlayer2FirstAction.setBounds(actionListPositionX(PLAYER2, FIRST_LIST_INDEX), actionListPositionY(),
				ACTION_SIZE_X, ACTION_SIZE_Y);
		...
		JPanel pnlPlayer2SecondAction = new JPanel()
		{
			public void paintComponent(Graphics g)
			{
				if (play.getIsOpenActionList()[SECOND_LIST_INDEX])
					g.drawImage(imgsrc.srcComboCards(play.getPlayer2Character().getID(),
							play.getPlayer2Character().getActionsID()[SECOND_LIST_INDEX]), 0, 0, null);
				else
					g.drawImage(imgsrc.srcBackCard(), 0, 0, null);
			}
		};
		pnlPlayer2SecondAction.setBounds(actionListPositionX(PLAYER2, SECOND_LIST_INDEX), actionListPositionY(),
				ACTION_SIZE_X, ACTION_SIZE_Y);
		...
		JPanel pnlPlayer2ThirdAction = new JPanel()
		{
			public void paintComponent(Graphics g)
			{
				if (play.getIsOpenActionList()[THIRD_LIST_INDEX])
					g.drawImage(imgsrc.srcComboCards(play.getPlayer2Character().getID(),
							play.getPlayer2Character().getActionsID()[THIRD_LIST_INDEX]), 0, 0, null);
				else
					g.drawImage(imgsrc.srcBackCard(), 0, 0, null);
			}
		};
		pnlPlayer2ThirdAction.setBounds(actionListPositionX(PLAYER2, THIRD_LIST_INDEX), actionListPositionY(),
				ACTION_SIZE_X, ACTION_SIZE_Y);
	...
	public void setMap()
	{
		...
		pnlPlayer1MapL = new JPanel()
		{
			public void paintComponent(Graphics g)
			{
				...
				if (play.getIsPlayer1ENUp())
				{
					...
					g.fillOval(PLAYER_ENUP_POSITION_X, PLAYER_ENUP_POSITION_Y[0], PLAYER_ENUP_SIZE_X,
							PLAYER_ENUP_SIZE_Y);
					g.fillOval(PLAYER_ENUP_POSITION_X, PLAYER_ENUP_POSITION_Y[1], PLAYER_ENUP_SIZE_X,
							PLAYER_ENUP_SIZE_Y);
					g.fillOval(PLAYER_ENUP_POSITION_X, PLAYER_ENUP_POSITION_Y[2], PLAYER_ENUP_SIZE_X,
							PLAYER_ENUP_SIZE_Y);
				}
			}
		};
		pnlPlayer1MapL.setBounds(largeMapPlayer1PositionX(play.getPlayer1Character().getX()),
				largeMapPositionY(play.getPlayer1Character().getY()), LARGE_MAP_CHARACTER_SIZE_X,
				LARGE_MAP_CHARACTER_SIZE_Y);
		add(pnlPlayer1MapL);
		...
		pnlPlayer2MapL.setBounds(largeMapPlayer2PositionX(play.getPlayer2Character().getX()),
				largeMapPositionY(play.getPlayer2Character().getY()), LARGE_MAP_CHARACTER_SIZE_X,
				LARGE_MAP_CHARACTER_SIZE_Y);
		add(pnlPlayer2MapL);
			...
			public void paintComponent(Graphics g)
			{
				...
				for (int i = 0; i < MAP_ROW; i++)
				{
					for (int j = 0; j < MAP_COLUMN; j++)
					{
						...
						{
							g.setColor(Color.red);
							g.fillRect((LARGE_MAP_RECT_SIZE_X + 1) * j, (LARGE_MAP_RECT_SIZE_Y + 1) * i,
									LARGE_MAP_RECT_SIZE_X, LARGE_MAP_RECT_SIZE_Y);
						} else
						{
							g.setColor(Color.black);
							g.drawRect((LARGE_MAP_RECT_SIZE_X + 1) * j, (LARGE_MAP_RECT_SIZE_Y + 1) * i,
									LARGE_MAP_RECT_SIZE_X, LARGE_MAP_RECT_SIZE_Y);
						}
					}
				}
	...
	{
		JPanel pnlWinPage = new JPanel()
		{
			@Override
			public void paintComponent(Graphics g)
			{
				if (player == PLAYER1)
					g.drawImage(imgsrc.srcPlayer1Win(), 0, 0, null);
				else if (player == PLAYER2)
					g.drawImage(imgsrc.srcPlayer2Win(), 0, 0, null);
			}
		};
		pnlWinPage.setSize(SCREEN_WIDTH, SCREEN_HEIGHT);
		add(pnlWinPage, 0);
		pnlPlayer1MapL.setVisible(false);
		pnlPlayer2MapL.setVisible(false);
		repaint();
	}
	...
	public void move(int player)
	{
		if (player == PLAYER1)
		{
			pnlPlayer1MapL.setBounds(play.getPlayer1X(), play.getPlayer1Y(), LARGE_MAP_CHARACTER_SIZE_X,
					LARGE_MAP_CHARACTER_SIZE_Y);
			pnlPlayer1MapL.repaint();
		}
		if (player == PLAYER2)
		{
			pnlPlayer2MapL.setBounds(play.getPlayer2X(), play.getPlayer2Y(), LARGE_MAP_CHARACTER_SIZE_X,
					LARGE_MAP_CHARACTER_SIZE_Y);
			pnlPlayer2MapL.repaint();
		}
	}
